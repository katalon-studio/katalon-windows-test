 ...
 * 2. Open `New` context menu at `Test Cases` folder
 * 3. Select `Test Case` by using accelerator key `T`
...
Windows.comment('2. Open `New` context menu at `Test Cases` folder')
TestCasesKeyword.openNewContextMenu()

Windows.comment('3. Select `Test Case` by using accelerator key `T``')
